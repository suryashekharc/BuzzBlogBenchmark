...
# Constants
COLUMNS = ["timestamp", "laddr", "lport", "raddr", "rport", "state"]
TCPRETRANS_LOG_PATTERN = r"^([0-9\.\-\:]+)\s+([^\s]+)\s+([^\s]+)\s+([^\s]+)\s+([^\s]+)\s+(.+)$"
...
class TcpretransParser:
  ...
  def extract_values_from_log(log):
    ...
    if not match:
      return None
    timestamp, laddr, lport, raddr, rport, state = match.groups()
    return (timestamp, laddr, lport, raddr, rport, state)
