...
# Constants
COLUMNS = ["timestamp", "request_id", "lservice", "lfunction", "key", "command", "latency"]
REDIS_LOG_PATTERN = r"^\[([0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]+)\] pid=([^\s]+) tid=([^\s]+) ls=([^\s]+) lf=([^\s]+) key=([^\s]+) cm=([^\s]+) rid=([^\s]+) lat=([^\s]+)$"
...
class RedisParser:
  ...
  def extract_values_from_log(log):
    ...
    if not match:
      return None
    timestamp, _, _, lservice, lfunction, key, command, request_id, latency = match.groups()
    return (timestamp, request_id, lservice, lfunction, key, command, latency)
