...
# Constants
COLUMNS = ["timestamp", "qlen_avg", "qlen_max", "qlat_avg", "qlat_max"]
CPURUNQFUNC_LOG_PATTERN = r"^([0-9\.\-\:]+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)$"


class CpurunqfuncParser:
  ...
  def extract_values_from_log(log):
    match = re.match(CPURUNQFUNC_LOG_PATTERN, log.strip())
    if not match:
      return None
    timestamp, qlen_avg, qlen_max, qlat_avg, qlat_max = match.groups()
    return (timestamp, qlen_avg, qlen_max, qlat_avg, qlat_max)
...
if __name__ == "__main__":
  ...
  with open(args.log_filepath) as logfile:
    CpurunqfuncParser.df(logfile).to_csv(args.csv_filepath, index=False)
