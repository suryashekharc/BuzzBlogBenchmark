...
# Constants
COLUMNS = ["timestamp", "lservice", "dbname", "latency"]
QUERY_CALL_LOG_PATTERN = r"^\[([0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]+)\] pid=([^\s]+) tid=([^\s]+) db=([^\s]+) ls=([^\s]+) lat=([^\s]+)$"


class QueryCallParser:
  ...
  def extract_values_from_log(log):
    match = re.match(QUERY_CALL_LOG_PATTERN, log)
    if not match:
      return None
    timestamp, _, _, dbname, lservice, latency = match.groups()
    return (timestamp, lservice, dbname, latency)
...
if __name__ == "__main__":
  ...
  with open(args.log_filepath) as logfile:
    QueryCallParser.df(logfile).to_csv(args.csv_filepath, index=False)
