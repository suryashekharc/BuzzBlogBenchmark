...
# Constants
COLUMNS = ["timestamp", "lservice", "rservice", "rfunction", "latency"]
RPC_CALL_LOG_PATTERN = r"^\[([0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]+)\] pid=([^\s]+) tid=([^\s]+) rs=([^\s]+) rf=([^\s]+) ls=([^\s]+) lat=([^\s]+)$"
...
class RPCCallParser:
  ...
  def extract_values_from_log(log):
    match = re.match(RPC_CALL_LOG_PATTERN, log)
    if not match:
      return None
    timestamp, _, _, rservice, rfunction, lservice, latency = match.groups()
    return (timestamp, lservice, rservice, rfunction, latency)
...
if __name__ == "__main__":
  ...
  with open(args.log_filepath) as logfile:
    RPCCallParser.df(logfile).to_csv(args.csv_filepath, index=False)
