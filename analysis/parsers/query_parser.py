...
# Constants
COLUMNS = ["timestamp", "request_id", "lservice", "lfunction", "dbname", "qtype", "latency"]
QUERY_LOG_PATTERN = r"^\[([0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]+)\] pid=([^\s]+) tid=([^\s]+) ls=([^\s]+) lf=([^\s]+) db=([^\s]+) qt=([^\s]+) rid=([^\s]+) lat=([^\s]+)$"
...
class QueryParser:
  ...
  def extract_values_from_log(log):
    ...
    if not match:
      return None
    timestamp, _, _, lservice, lfunction, dbname, qtype, request_id, latency = match.groups()
    return (timestamp, request_id, lservice, lfunction, dbname, qtype, latency)
