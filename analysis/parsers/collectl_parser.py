...
HW_METRICS = {
    ...
    "dsk": ["name", "reads", "rmerge", "rkbytes", "waitr", "writes", "wmerge", "wkbytes", "waitw", "request", "quelen", "wait", "svctim", "util"]
}
COLUMNS = {
...
FILE_EXTENSION_TO_HW_TYPE = {"cpu": "cpu", "numa": "mem", "dsk": "dsk"}
...
class CollectlParser:
  ...
  def df(cls, logfile, hw_type):
    for log in logfile:
      if log[0] == '#':
        ...
        break
    df = pd.DataFrame([entry for log in logfile for entry in cls.extract_values_from_log(log, hw_type, offset)], columns=COLUMNS[hw_type])
    for hw_no in df.hw_no.unique():
      if (hw_type == "cpu" and df[(df["hw_no"] == hw_no)]["total"].unique().size == 1) or \
          ...
          (hw_type == "dsk" and df[(df["hw_no"] == hw_no)]["reads"].unique().size == 1 and df[(df["hw_no"] == hw_no)]["writes"].unique().size == 1):
  ...
  def extract_values_from_log(log, hw_type, offset):
    ...
    for hw_no in range((len(values) - 2) // len(HW_METRICS[hw_type])):
      yield (timestamp, hw_no, *values[2 + hw_no * len(HW_METRICS[hw_type]):2 + (hw_no + 1) * len(HW_METRICS[hw_type])])
...
if __name__ == "__main__":
  ...
  with gzip.open(args.log_filepath, "rt") as logfile:
    CollectlParser.df(logfile, FILE_EXTENSION_TO_HW_TYPE[args.log_filepath.split('.')[-2]]).\
