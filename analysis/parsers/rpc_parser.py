...
# Constants
COLUMNS = ["timestamp", "request_id", "lservice", "lfunction", "rservice", "rfunction", "latency"]
RPC_LOG_PATTERN = r"^\[([0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]+)\] pid=([^\s]+) tid=([^\s]+) ls=([^\s]+) lf=([^\s]+) rs=([^\s]+) rf=([^\s]+) rid=([^\s]+) lat=([^\s]+)$"
...
class RPCParser:
  ...
  def extract_values_from_log(log):
    ...
    if not match:
      return None
    timestamp, _, _, lservice, lfunction, rservice, rfunction, request_id, latency = match.groups()
    return (timestamp, request_id, lservice, lfunction, rservice, rfunction, latency)
