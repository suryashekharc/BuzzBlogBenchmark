...
# Constants
COLUMNS = ["timestamp", "lservice", "dbname", "backlog", "latency"]
QUERY_CONN_LOG_PATTERN = r"^\[([0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]+)\] pid=([^\s]+) tid=([^\s]+) ls=([^\s]+) db=([^\s]+) bl=([^\s]+) lat=([^\s]+)$"
...
class QueryConnParser:
  ...
  def extract_values_from_log(log):
    ...
    if not match:
      return None
    timestamp, _, _, lservice, dbname, backlog, latency = match.groups()
    return (timestamp, lservice, dbname, backlog, latency)
