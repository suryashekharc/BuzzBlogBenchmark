...
int main(int argc, char *argv[]) {
  ...
  if (argc != 6) {
    printf(
        "Usage: cpuinterference [min_threads] [max_threads] [thread_step] "
        "[period_in_sec] [duration_in_ms]\n");
    exit(1);
  }
  ...
  while (1) {
    for (int n_threads = min_threads; n_threads <= max_threads;
         n_threads += thread_step) {
      // Set flag to 0.
      flag = 0;
      // Wait period.
      ...
      while (1) {
        ...
        if (elapsed > period_in_sec) break;
        sleep(1);
      }
      ...
      while (1) {
        ...
        elapsed =
            (now.tv_sec - start.tv_sec) + 1e-9 * (now.tv_nsec - start.tv_nsec);
        if (elapsed * 1000 > duration_in_ms) break;
      }
      // Set flag to 1.
      flag = 1;
      // Join threads.
