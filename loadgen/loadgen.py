...
class BuzzBlogSession(ATLoad.Session):
  ...
  def _request(self, method, path, params=None, json=None):
    ...
    with eventlet.Timeout(30, False):
      r = getattr(requests, method)(self._url_prefix + path,
                                    auth=auth,
                                    params=params,
                                    json=json,
                                    timeout=30)
    ...
    self._log(
        LOG_PATTERN.format(ts=start_time.strftime("%Y-%m-%d %H:%M:%S.%f")[:-3],
                           method=method.upper(),
  ...
  def create_post(self):
    r = self._request(
        "post",
        "/post",
        json={"text": _random_string(128) + " " + random.choice(HASHTAGS)})
    if r.status_code == 200:
  ...
  def retrieve_recent_posts(self):
    r = self._request("get", "/post", params={"limit": 1, "offset": 0})
  ...
  def retrieve_account_posts(self):
    if self._other_account:
      r = self._request("get",
                        "/post",
                        params={
                            "author_id": self._other_account["id"],
                            "limit": 1,
                            "offset": 0
                        })
      if r.status_code == 200 and r.json():
  ...
  def retrieve_account_followers(self):
    if self._other_account:
      r = self._request("get",
                        "/follow",
                        params={
                            "followee_id": self._other_account["id"],
                            "limit": 1,
                            "offset": 0
                        })
      if r.status_code == 200 and r.json():
  ...
  def retrieve_account_followees(self):
    if self._other_account:
      r = self._request("get",
                        "/follow",
                        params={
                            "follower_id": self._other_account["id"],
                            "limit": 1,
                            "offset": 0
                        })
      if r.status_code == 200 and r.json():
  ...
  def retrieve_account_likes(self):
    if self._other_account:
      self._request("get",
                    "/like",
                    params={
                        "account_id": self._other_account["id"],
                        "limit": 1,
                        "offset": 0
                    })

  def list_trending_hashtags(self):
    r = self._request("get", "/trending", params={"limit": 10})
...
if __name__ == "__main__":
  ...
  parser.add_argument("--workload_conf",
                      required=True,
                      action="store",
                      type=str,
  ...
  workload = ATLoad.Workload(args.workload_conf, args.log, BuzzBlogSession,
                             int(args.seed), int(args.n_workers), args.hostname,
                             args.port)
  random.seed(None)
  workload.run()
