...
class SSHClient:
  ...
  def __init__(self, hostname, port, username, key_filename, log_filename, max_sessions=40):
    ...
    self._client.connect(hostname=hostname, port=port, username=username, key_filename=key_filename)
  ...
  def exec(self, command, retries=10):
    ...
    Data written to stdout and stderr are appended to files named with the log_filename attribute and extensions '.out' and '.err', respectively.
    """
    with self._semaphore:
      print("[{ts}][{hostname}] {command}".format(ts=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), hostname=self._hostname, command=command))
      while retries + 1 > 0:
        try:
