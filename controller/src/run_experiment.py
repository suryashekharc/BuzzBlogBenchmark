...
LOG_FILENAME_TO_PARSER = {
    ...
    "query.log": "/opt/BuzzBlogBenchmark/analysis/parsers/query_parser.py",
    "query_call.log": "/opt/BuzzBlogBenchmark/analysis/parsers/query_call_parser.py",
    "query_conn.log": "/opt/BuzzBlogBenchmark/analysis/parsers/query_conn_parser.py",
    "redis.log": "/opt/BuzzBlogBenchmark/analysis/parsers/redis_parser.py",
    "rpc(_[0-9]+)?.log": "/opt/BuzzBlogBenchmark/analysis/parsers/rpc_parser.py",
    "rpc_call(_[0-9]+)?.log": "/opt/BuzzBlogBenchmark/analysis/parsers/rpc_call_parser.py",
    "rpc_conn(_[0-9]+)?.log": "/opt/BuzzBlogBenchmark/analysis/parsers/rpc_conn_parser.py",
}
...
def build_backend_conf():
  ...
  for node_hostname, node_conf in SYS_CONF.items():
    for container_name, container_conf in node_conf.get("containers", {}).items():
      if container_name.endswith("_service") or container_name.endswith("_database") or container_name.endswith("_redis"):
        container_basename = container_name[:container_name.find('_')]
        container_type = container_name[container_name.find('_') + 1:]
        container_addr = node_hostname + ":" + container_conf["options"]["publish"].split(':')[0]
        if container_basename not in BACKEND_CONF:
          BACKEND_CONF[container_basename] = {"service": []}
        if container_type == "service":
          BACKEND_CONF[container_basename][container_type].append(container_addr)
...
def start_container(node_hostname, node_conf, container_name, ssh_client):
  ...
  ssh_client.exec(
      "sudo docker run " + ("--volume /tmp/%s:/tmp " % container_name) + " ".join([
          "--%s %s" % (param, value) if isinstance(value, str) else " ".join(["--%s %s" % (param, v)
                                                                              for v in value])
  ...
  if container_conf["image"].startswith("postgres"):
    ...
    subprocess.run("psql -U postgres -d %s -h %s -p %s -f %s" %
                   (container_name.split('_')[0], node_hostname, container_conf["options"]["publish"].split(':')[0],
                    "/opt/BuzzBlog/app/{service}/database/{service}_schema.sql".format(service=container_name.split('_')[0])),
                   shell=True)
...
def all_nodes(func):
  ...
  def func_wrapper(*args, **kwargs):
    threads = []
    for node_hostname, node_conf in SYS_CONF.items():
      ssh_client = SSHClient(node_hostname, node_conf["ssh"]["port"], node_conf["ssh"]["username"], node_conf["ssh"]["key_filename"],
                             os.path.join(DIRNAME, "ssh", "%s-%s" % (node_hostname, func.__name__)))
      threads.append(threading.Thread(target=func, args=[node_hostname, node_conf, ssh_client]))
...
def configure_kernel(node_hostname, node_conf, ssh_client):
  ssh_client.exec(" && ".join(["sudo sysctl -w %s=\"%s\"" % (param, value) for param, value in node_conf.get("kernel", {}).items()]))


@all_nodes
def save_system_specs(node_hostname, node_conf, ssh_client):
  with open(os.path.join(DIRNAME, "specs", node_hostname, "hw"), "wb+") as hw_file:
    hw_file.write(ssh_client.exec("sudo lshw")[0])
  with open(os.path.join(DIRNAME, "specs", node_hostname, "cpu"), "wb+") as cpu_file:
    cpu_file.write(ssh_client.exec("lscpu")[0])
  with open(os.path.join(DIRNAME, "specs", node_hostname, "mem"), "wb+") as mem_file:
    mem_file.write(ssh_client.exec("lsmem")[0])
  with open(os.path.join(DIRNAME, "specs", node_hostname, "blk"), "wb+") as blk_file:
    blk_file.write(ssh_client.exec("lsblk")[0])
  with open(os.path.join(DIRNAME, "specs", node_hostname, "kernel"), "wb+") as kernel_file:
...
def install_buzzblogbenchmark(node_hostname, node_conf, ssh_client):
  VERSION = "1.0"
  ssh_client.exec("sudo rm -rf /opt/BuzzBlogBenchmark && "
...
def install_pandas(node_hostname, node_conf, ssh_client):
  ssh_client.exec("sudo apt-get update && "
                  "sudo DEBIAN_FRONTEND=noninteractive apt-get -y install "
                  "python3-pip && "
...
def pull_docker_images(node_hostname, node_conf, ssh_client):
  if DOCKER_HUB_USERNAME and DOCKER_HUB_PASSWORD:
    ssh_client.exec("sudo docker login -u {username} -p {password}".format(username=DOCKER_HUB_USERNAME, password=DOCKER_HUB_PASSWORD))
  threads = []
  for container_image in set([container_conf["image"].split(' ')[0] for container_conf in node_conf.get("containers", {}).values()]):
    threads.append(threading.Thread(target=ssh_client.exec, args=["sudo docker pull %s" % container_image]))
  for thread in threads:
    thread.start()
  for thread in threads:
    thread.join()
...
def copy_workload_configuration_file(node_hostname, node_conf, ssh_client):
  ssh_client.exec("sudo mkdir -p /usr/local/etc/loadgen && "
                  "echo \"{content}\" | sudo tee {filepath}".format(content=yaml.dump(WL_CONF), filepath="/usr/local/etc/loadgen/workload.yml"))
...
def render_configuration_templates(node_hostname, node_conf, ssh_client):
  ...
  for template_name, template_conf in node_conf.get("templates", {}).items():
    ...
    ssh_client.exec("echo \"{content}\" | sudo tee {filepath}".format(content=env.get_template(template_name).render(**template_conf["params"]).replace(
        '"', '\\"'),
...
def generate_backend_configuration_file(node_hostname, node_conf, ssh_client):
  ssh_client.exec("sudo mkdir -p /etc/opt/BuzzBlog && "
                  "echo \"{content}\" | sudo tee {filepath}".format(content=yaml.dump(BACKEND_CONF), filepath="/etc/opt/BuzzBlog/backend.yml"))
...
def start_monitors(node_hostname, node_conf, ssh_client):
  for monitor_name, monitor_conf in node_conf["monitors"].items():
    ...
    ssh_client.exec("sudo nohup nice -n %s " % monitor_conf.get("niceness", 19) + "stdbuf -oL -eL " + monitor_conf.get("command", monitor_name) + " " +
                    " ".join(["--%s %s" % (param, value) for (param, value) in monitor_conf.get("options", {}).items()]) + " " +
...
def start_containers():
  ...
  for node_hostname, node_conf in SYS_CONF.items():
    ssh_client = SSHClient(node_hostname, node_conf["ssh"]["port"], node_conf["ssh"]["username"], node_conf["ssh"]["key_filename"],
                           os.path.join(DIRNAME, "ssh", "%s-%s" % (node_hostname, "start_containers")))
    for (container_name, container_conf) in node_conf.get("containers", {}).items():
      containers.append((container_conf["start_order"], node_hostname, node_conf, container_name, ssh_client))
  for current_start_order in sorted(set([container[0] for container in containers])):
    ...
    for (start_order, node_hostname, node_conf, container_name, ssh_client) in containers:
      if start_order == current_start_order:
        threads.append(threading.Thread(target=start_container, args=[node_hostname, node_conf, container_name, ssh_client]))
    for thread in threads:
      thread.start()
    for thread in threads:
      thread.join()
...
def stop_monitors(node_hostname, node_conf, ssh_client):
  for monitor_name, monitor_conf in node_conf["monitors"].items():
    ssh_client.exec("sudo pkill %s" % monitor_conf.get("command", monitor_name).split(' ')[0])
...
def fetch_monitoring_data(node_hostname, node_conf, ssh_client):
  for monitor_name, monitor_conf in node_conf["monitors"].items():
    if PARSE_LOG_FILES:
      ...
      if monitor_name == "cpurunq-bpftrace":
        ssh_client.exec("python3 /opt/BuzzBlogBenchmark/analysis/parsers/cpurunq_parser.py "
                        "--log_filepath {dirpath}/log --csv_filepath {dirpath}/log.csv".format(dirpath=monitor_conf["dirpath"]))
      if monitor_name == "cpurunqfunc-bpftrace":
        ssh_client.exec("python3 /opt/BuzzBlogBenchmark/analysis/parsers/cpurunqfunc_parser.py "
                        "--log_filepath {dirpath}/log --csv_filepath {dirpath}/log.csv".format(dirpath=monitor_conf["dirpath"]))
      ...
      if monitor_name == "tcpretrans-bpftrace":
        ssh_client.exec("python3 /opt/BuzzBlogBenchmark/analysis/parsers/tcpretrans_parser.py "
                        "--log_filepath {dirpath}/log --csv_filepath {dirpath}/log.csv".format(dirpath=monitor_conf["dirpath"]))
    ssh_client.exec("tar -C {dirpath} -czf /tmp/{monitor_name}.tar.gz .".format(monitor_name=monitor_name, dirpath=monitor_conf["dirpath"]))
    ssh_client.copy("/tmp/{monitor_name}.tar.gz".format(monitor_name=monitor_name), os.path.join(DIRNAME, "logs", node_hostname))
...
def fetch_container_logs(node_hostname, node_conf, ssh_client):
  for container_name, container_conf in node_conf["containers"].items():
    ...
    ssh_client.exec("sudo docker logs {container_name} > "
                    "{dirpath}/{container_name}.log 2>&1".format(container_name=container_name, dirpath=dirpath))
    if PARSE_LOG_FILES:
        ...
        for filename_pat in LOG_FILENAME_TO_PARSER:
          if re.match(filename_pat, log_filename):
            ssh_client.exec("python3 {parser_path} "
                            ...
                            "--csv_filepath {csv_filepath}".format(parser_path=LOG_FILENAME_TO_PARSER[filename_pat],
                                                                   log_filepath=os.path.join(dirpath, log_filename),
                                                                   csv_filepath=os.path.join(dirpath,
                                                                                             os.path.splitext(log_filename)[0] + ".csv")))
    ssh_client.exec("tar -C {dirpath} -czf {dirpath}.tar.gz .".format(dirpath=dirpath))
    ssh_client.copy("{dirpath}.tar.gz".format(dirpath=dirpath), os.path.join(DIRNAME, "logs", node_hostname))
...
def main():
  ...
  parser.add_argument("--description", required=True, action="store", type=str, help="Experiment description")
  parser.add_argument("--system_conf", required=True, action="store", type=str, help="Path to the system configuration file")
  parser.add_argument("--workload_conf", required=False, default="", action="store", type=str, help="Path to the workload configuration file")
  parser.add_argument("--docker_hub_username", required=False, default="", action="store", help="Docker Hub username")
  parser.add_argument("--docker_hub_password", required=False, default="", action="store", help="Docker Hub password")
  ...
  for system_conf in system_confs:
    ...
    for (i, workload_conf) in enumerate(workload_confs):
      ...
      if i == 0:
        ...
        configure_kernel()
        # Stop monitors and containers of any previous execution.
        stop_monitors()
        stop_containers()
      ...
      global METADATA
      METADATA = {}
      update_metadata({"user": subprocess.getoutput("whoami"), "start_time": timestamp(), "description": args.description})
      ...
      start_containers()
      start_monitors()
      time.sleep(int(workload_conf["duration"]["total"]))
      stop_monitors()
      stop_containers()
